...
if (mode === "command") {
	const rows = readFileSync(path.join(import.meta.dirname, "sample.csv"), "utf8")
		.trim()
		.split(/\r?\n/)
		.slice(1);
	...
	process.stdout.write(
		JSON.stringify({
			artifacts: { file: path.relative(root, report) },
			cleanup: { ok: true },
			rows: rows.length,
			total,
		}),
	...
	process.stdout.write(
		JSON.stringify({
			artifacts: { log: path.relative(root, log) },
			cleanup: { ok: true, listener: "closed" },
			status: response.status,
			body,
		}),
	);
