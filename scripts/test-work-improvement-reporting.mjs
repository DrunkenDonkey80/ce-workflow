...
try {
	...
	assert(
		mirrorResult.taskId !== result.taskId,
		"independent clones allocate collision-resistant report ids",
	);
	const store = loadStore(source);
	assert(
		store.items[result.epicId].title === "Self-improving",
		"creates exact epic",
	);
	assert(
		store.items[result.taskId].parentId === result.epicId,
		"creates epic child",
	);
	assert(
		!JSON.stringify(store.items[result.taskId]).includes(log),
	...
	const manifest = JSON.parse(
		readFileSync(path.join(result.bundlePath, "manifest.json"), "utf8"),
	);
	assert(
		manifest.files.length === 2 && manifest.files[0].source === log,
	...
	if (process.platform !== "win32") {
		assert(
			(statSync(result.bundlePath).mode & 0o777) === 0o700,
			"bundle is owner-only",
		);
		assert(
			(statSync(path.join(result.bundlePath, "manifest.json")).mode & 0o777) ===
				0o600,
	...
	assert(
		...
		"bundle destination is ignored",
	);

	const submit = (nextReport, extra = {}) =>
		submitImprovementReport({
			cwd: consumer,
			packageRoot: source,
			settings: { workImprovement: { sourceCheckout: source } },
	...
	const relative = await submit({
		...
		logs: [path.relative(consumer, log)],
	});
	assert(
		relative.taskId,
		"relative evidence paths resolve from the consumer project",
	);
	...
	assert(
		provenance.producer.length <= 80 && !/[\\/]/.test(provenance.producer),
		"producer provenance is safe and bounded",
	);
	assert(provenance.workflowId.length <= 120, "workflow provenance is bounded");
	mutateStore(source, (next) => {
		next.items[result.taskId].status = "closed";
	});
	cleanupImprovementReportBundles(source);
	assert(
		!existsSync(result.bundlePath),
		"closed report cleanup removes its evidence bundle",
	);
	...
	cleanupImprovementReportBundles(source);
	assert(
		!existsSync(stale) && existsSync(fresh),
		"cleanup removes only stale unreferenced bundles",
	);
	...
	const concurrent = await Promise.all([
		submit(
			{ ...report, observation: "Concurrent A" },
			{ withFileMutationQueue: concurrentQueue },
		),
		submit(
			{ ...report, observation: "Concurrent B" },
			{ withFileMutationQueue: concurrentQueue },
		),
	]);
	assert(
		concurrent[0].taskId !== concurrent[1].taskId,
		"concurrent submissions create distinct tasks",
	);
	assert(
		concurrent[0].epicId === concurrent[1].epicId,
	...
	const cleanupFailure = await submit(
		{ ...report, observation: "Cleanup failure" },
		{
			_cleanupBundles: () => {
	...
	assert(
		cleanupFailure.taskId,
		"retention failure does not block report intake",
	);

	const beforeFailures = Object.values(loadStore(source).items).filter(
		(item) => item.type === "task",
	).length;
	...
	for (const [candidate, message] of [
		...
		await submit({ ...report, logs: [candidate] }).then(
			...
			(error) =>
				assert(/evidence/.test(error.message), `${message} fails clearly`),
		);
	}
	await submit({ ...report, logs: [path.join(root, "outside.log")] }).then(
		...
		(error) =>
			assert(
				/approved evidence root/.test(error.message),
				"unrelated evidence fails clearly",
			),
	);
	...
	if (process.platform !== "win32") {
		...
		await submit({ ...report, logs: [linkedFile] }).then(
			...
			(error) =>
				assert(/symlink/.test(error.message), "direct symlink fails clearly"),
		);
	}
	await submit({ ...report, logs: [path.join(linkedDir, "secret.log")] }).then(
		() => assert(false, "symlinked parent evidence must fail"),
		(error) =>
			assert(/symlink/.test(error.message), "symlinked parent fails clearly"),
	);
	let capturedFd;
	await submit(
		{ ...report, observation: "Setup failure", logs: [log] },
		{
			_beforeCopy: ([input]) => {
				capturedFd = input.fd;
		...
		(error) =>
			assert(
				/injected setup failure/.test(error.message),
				"setup failure propagates clearly",
			),
	);
	...
	await submit({ ...report, logs: [oversized] }).then(
		...
		(error) =>
			assert(/exceeds/.test(error.message), "oversized evidence fails clearly"),
	);
	...
	await submit(
		...
		(error) =>
			assert(
				...
				"same-size rewritten evidence fails clearly",
			),
	);
	await submit(
		{ ...report, observation: "Rename failure" },
		{
			_renameBundle: () => {
		...
		(error) =>
			assert(
				/injected rename failure/.test(error.message),
				"rename failure propagates clearly",
			),
	);
	await submit(
		{ ...report, observation: "No-op rename" },
		{ _renameBundle: () => {} },
	).then(
		() => assert(false, "no-op rename must not create a task"),
		(error) =>
			assert(
				/finalized/.test(error.message),
				"no-op rename fails the complete-bundle check",
			),
	);
	...
	try {
		await submit({ ...report, observation: "Unsafe destination" }).then(
			...
			(error) =>
				assert(
					/destination.*symlink/i.test(error.message),
					"symlinked destination fails clearly",
				),
		);
	} finally {
	...
	await submit(
		{ ...report, observation: "Store failure" },
		{
		...
		(error) =>
			assert(
				...
				"store failure propagates clearly",
			),
	);
	assert(
		Object.values(loadStore(source).items).filter(
	...
	writeFileSync(
		...
		JSON.stringify({
			...
			workImprovement: { sourceCheckout: source },
		}),
	);
	await hooks.session_start(
		{},
		{ cwd: consumer, ui: { notify() {}, setStatus() {}, setTitle() {} } },
	);
	assert(
