...
const regressedCost = pairs(80);
for (const pair of regressedCost) pair.attempts[0].candidate.metrics.toolCalls = 111;
assert.equal(evaluateDecision(regressedCost, rubric).reason, "cost-dimension-regression");
assert.equal(evaluateDecision(regressedCost, rubric, { maximumDimensionRegression: 0.12 }).status, "candidate-accepted");
const missing = pairs();
delete missing[0].attempts[0].candidate.metrics.toolOutputChars;
assert.equal(evaluateDecision(missing, rubric).status, "invalid");
const evaluatorFailure = pairs();
evaluatorFailure[0].attempts[0].evaluatorFailure = "malformed";
assert.equal(evaluateDecision(evaluatorFailure, rubric).status, "invalid");
const provenanceFailure = pairs();
provenanceFailure[0].attempts[0].comparisonFailure = "provenance-mismatch";
assert.equal(evaluateDecision(provenanceFailure, rubric).reason, "provenance-mismatch");
...
process.stdout.write("ok - workflow evaluation decision scoring fixtures\n");
