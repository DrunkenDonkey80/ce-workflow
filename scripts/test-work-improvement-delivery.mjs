...
function fixture(name) {
	const root = mkdtempSync(
		path.join(tmpdir(), `work-improvement-delivery-${name}-`),
	);
	roots.push(root);
...
async function deliver(name, behavior = {}) {
	...
	const runGit = (cwd, args) => {
		...
		if (behavior.refDeleteFail && args[0] === "update-ref" && args[1] === "-d")
		...
		if (args[0] === "push") {
			...
			if (behavior.pushAbsent)
				throw new Error("simulated timeout before remote received push");
...
try {
	...
	assert.equal(
		git(accepted.repo.source, "rev-parse", "HEAD"),
		accepted.repo.commitSha,
	);
	assert.equal(
		git(accepted.repo.source, "rev-parse", "@{upstream}"),
		accepted.repo.commitSha,
	);
	assert.equal(
		existsSync(
			path.join(accepted.repo.source, ".git", accepted.repo.candidateRef),
		),
		false,
	);
	const events = readFileSync(
		path.join(
			accepted.repo.source,
			".pi",
			"work-improvement",
	...
	assert.equal(
		events.find(
			(event) => event.transition === "push-pending" && event.integrationSha,
		)?.integrationSha,
		accepted.repo.commitSha,
	);
	...
	assert.equal(
		releaseLease(accepted.repo.source, accepted.repo.lease.lease).ok,
		true,
	);
	...
	assert.equal(
		ambiguous.pushCalls,
		1,
		"present remote SHA must suppress duplicate push",
	);
	assert.equal(
		releaseLease(ambiguous.repo.source, ambiguous.repo.lease.lease).ok,
		true,
	);
	...
	assert.equal(
		absent.pushCalls,
		2,
		"a proven absent push gets one bounded safe retry",
	);
	assert.equal(
		git(absent.repo.source, "rev-parse", "@{upstream}"),
		absent.repo.expected.head,
	);
	assert.equal(
		git(absent.repo.source, "rev-parse", "--verify", absent.repo.candidateRef),
		absent.repo.commitSha,
	);
	...
	assert.equal(
		resumed.state,
		"accepted",
		"persisted integration SHA resumes without duplicate integration",
	);
	assert.equal(
		releaseLease(absent.repo.source, absent.repo.lease.lease).ok,
		true,
	);

	const dirtyIntegration = await deliver("dirty-integration", {
		dirtyBeforeIntegration: true,
	});
	assert.equal(dirtyIntegration.result.state, "deferred");
	assert.equal(dirtyIntegration.result.reason, "dirty-worktree");
	assert.equal(
		git(dirtyIntegration.repo.source, "rev-parse", "HEAD"),
		dirtyIntegration.repo.expected.head,
	);
	assert.equal(
		releaseLease(
			dirtyIntegration.repo.source,
			dirtyIntegration.repo.lease.lease,
		).ok,
		true,
	);

	const branchIntegration = await deliver("branch-integration", {
		branchBeforeIntegration: true,
	});
	assert.equal(branchIntegration.result.state, "deferred");
	assert.equal(branchIntegration.result.reason, "branch-changed");
	assert.equal(
		git(branchIntegration.repo.source, "rev-parse", "HEAD"),
		branchIntegration.repo.expected.head,
	);
	assert.equal(
		releaseLease(
			branchIntegration.repo.source,
			branchIntegration.repo.lease.lease,
		).ok,
		true,
	);
	...
	assert.equal(
		git(
			diverged.repo.source,
			"rev-parse",
			"--verify",
			diverged.repo.candidateRef,
		),
		diverged.repo.commitSha,
	);
	assert.equal(
		releaseLease(diverged.repo.source, diverged.repo.lease.lease).ok,
		true,
	);
	...
	assert.match(
		git(reverted.repo.source, "log", "-1", "--pretty=%s"),
		/^Revert /,
	);
	assert.equal(
		git(reverted.repo.source, "rev-parse", "HEAD"),
		reverted.result.revertSha,
	);
	assert.equal(
		git(reverted.repo.source, "rev-parse", "@{upstream}"),
		reverted.result.revertSha,
	);
	assert.equal(
		readFileSync(path.join(reverted.repo.source, "lib.txt"), "utf8").trim(),
		"before",
	);
	assert.equal(
		releaseLease(reverted.repo.source, reverted.repo.lease.lease).ok,
		true,
	);

	const dirty = await deliver("dirty-revert", {
		validationFail: true,
		dirtyBeforeRevert: true,
	});
	...
	assert.equal(
		git(dirty.repo.source, "rev-parse", "--verify", dirty.repo.candidateRef),
		dirty.repo.commitSha,
	);
	assert.equal(
		releaseLease(dirty.repo.source, dirty.repo.lease.lease).ok,
		true,
	);
	...
	assert.equal(
		git(
			branchRace.repo.remote,
			"rev-parse",
			`refs/heads/${branchRace.repo.expected.branch}`,
		),
	...
	assert.equal(
		branchRaceRestart.state,
		"accepted",
		"push ambiguity reconciles on restart",
	);
	assert.equal(
		releaseLease(branchRace.repo.source, branchRace.repo.lease.lease).ok,
		true,
	);
	...
	assert.equal(
		git(
			advanced.repo.source,
			"rev-parse",
			"--verify",
			advanced.repo.candidateRef,
		),
		advanced.repo.commitSha,
	);
	assert.equal(
		releaseLease(advanced.repo.source, advanced.repo.lease.lease).ok,
		true,
	);
	...
	assert.equal(
		git(
			retargeted.repo.source,
			"rev-parse",
			"--verify",
			retargeted.repo.candidateRef,
		),
		retargeted.repo.expected.head,
		"terminal cleanup must not delete a replacement ref",
	);
	assert.equal(
		releaseLease(retargeted.repo.source, retargeted.repo.lease.lease).ok,
		true,
	);

	const cleanupFailed = await deliver("validation-cleanup", {
		validationCleanupFail: true,
	});
	assert.equal(cleanupFailed.result.state, "manual-recovery");
	assert.equal(
		cleanupFailed.result.reason,
		"validation-worktree-cleanup-failed",
	);
	assert.match(
		git(cleanupFailed.repo.source, "worktree", "list", "--porcelain"),
		/validation-attempt-validation-cleanup-/,
	);
	assert.equal(
		git(
			...
			"rev-parse",
			"--verify",
			cleanupFailed.repo.candidateRef,
		),
		cleanupFailed.repo.commitSha,
	);
	assert.equal(
		releaseLease(cleanupFailed.repo.source, cleanupFailed.repo.lease.lease).ok,
		true,
	);

	const hanging = await deliver("hanging-validation", {
		hangingValidation: true,
	});
	...
	assert.equal(
		git(
			...
			"rev-parse",
			"--verify",
			hanging.repo.candidateRef,
		),
		hanging.repo.commitSha,
	);
	assert.equal(
		releaseLease(hanging.repo.source, hanging.repo.lease.lease).ok,
		true,
	);

	const cancelled = await deliver("cancelled-validation", {
		cancelledValidation: true,
	});
	assert.equal(
		cancelled.result.state,
		"reverted",
		"confirmed cancellation may clean up and revert",
	);
	assert.equal(cancelled.validationSignal.aborted, true);
	assert.doesNotMatch(
		git(cancelled.repo.source, "worktree", "list", "--porcelain"),
		/validation-attempt-cancelled-validation-/,
	);
	assert.equal(
		releaseLease(cancelled.repo.source, cancelled.repo.lease.lease).ok,
		true,
	);

	const cleanupPending = await deliver("cleanup-pending", {
		refDeleteFail: true,
	});
	...
	const cleanupPendingRestart = await runImprovementDelivery(
		cleanupPending.options,
		{
	...
	assert.throws(
		() =>
			git(
				cleanupPending.repo.source,
				"rev-parse",
				"--verify",
				cleanupPending.repo.candidateRef,
			),
		/needed a single revision|Command failed/,
	);
	assert.equal(
		releaseLease(cleanupPending.repo.source, cleanupPending.repo.lease.lease)
			.ok,
		true,
	);
	...
	const seedRecovery = (repo, candidateId, state, details = {}) => {
		...
		appendCandidateTransition(repo.source, candidateId, state, {
			...metadata,
			...details,
		});
	};
	...
	assert.deepEqual(
		await reconcileAutonomousImprovement(liveRecovery.source),
		[],
		"a second live session cannot take the writer lease",
	);
	assert.equal(
		readCandidateState(liveRecovery.source).events.length,
		liveEventCount,
		"live-owner recovery must not rewrite candidate state",
	);
	assert.equal(
		releaseLease(liveRecovery.source, liveRecovery.lease.lease).ok,
		true,
	);
	...
	assert.equal(
		releaseLease(pushRecovery.source, pushRecovery.lease.lease).ok,
		true,
	);
	assert.deepEqual(
		await reconcileAutonomousImprovement(
			pushRecovery.source,
			{},
			{
				runGit: (cwd, args) => git(cwd, ...args),
	...
	assert.equal(
		readCandidateState(pushRecovery.source).candidates.get(
			"candidate-restart-push",
		).state,
		"accepted",
		"restart resumes push and validation",
	);

	const revertRecovery = fixture("restart-revert");
	seedRecovery(revertRecovery, "candidate-restart-revert", "committed");
	assert.equal(
		releaseLease(revertRecovery.source, revertRecovery.lease.lease).ok,
		true,
	);
	await reconcileAutonomousImprovement(
		revertRecovery.source,
		{},
		{
			runGit: (cwd, args) => git(cwd, ...args),
			runPackageVerify: async () => ({ passed: true }),
	...
	assert.equal(
		readCandidateState(revertRecovery.source).candidates.get(
			"candidate-restart-revert",
		).state,
		"reverted",
		"restart resumes validation failure through revert cleanup",
	);

	const cleanupRecovery = fixture("restart-cleanup");
	git(
		cleanupRecovery.source,
		"merge",
		"--ff-only",
		cleanupRecovery.candidateRef,
	);
	git(cleanupRecovery.source, "push");
	seedRecovery(
		cleanupRecovery,
		"candidate-restart-cleanup",
		"cleanup-pending",
		{
			integrationSha: cleanupRecovery.commitSha,
			remoteSha: cleanupRecovery.commitSha,
			cleanupState: "accepted",
		},
	);
	assert.equal(
		releaseLease(cleanupRecovery.source, cleanupRecovery.lease.lease).ok,
		true,
	);
	await reconcileAutonomousImprovement(
		cleanupRecovery.source,
	...
	assert.equal(
		readCandidateState(cleanupRecovery.source).candidates.get(
			"candidate-restart-cleanup",
		).state,
		"accepted",
		"restart finishes durable cleanup intent",
	);
	assert.throws(
		() =>
			git(
