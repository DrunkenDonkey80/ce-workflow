...
export function recoverStaleWorkspaces(tempRoot = os.tmpdir(), maxAgeMs = 24 * 60 * 60 * 1000, now = Date.now()) {
	const removed = [];
	for (const name of readdirSync(tempRoot).filter((entry) => entry.startsWith("ce-workflow-samples-"))) {
		const directory = path.join(tempRoot, name);
		const leaseFile = path.join(directory, ".active.json");
		let lease = null;
		try { lease = JSON.parse(readFileSync(leaseFile, "utf8")); } catch {}
		const age = now - Number(lease?.startedAt ?? statSync(directory).mtimeMs);
		let alive = false;
		if (Number.isInteger(lease?.pid)) {
			try { process.kill(lease.pid, 0); alive = true; } catch {}
		}
...
function markRunRoot(runRoot) {
	...
	writeFileSync(path.join(runRoot, ".active.json"), `${JSON.stringify({ pid: process.pid, startedAt: Date.now() })}\n`);
}
...
function treeHash(root) { return hashTree(root, (_relative, name) => name === ".git" || name === ".ce-workflow"); }
...
function sourceState(root) {
	...
	if (status.status !== 0 || listed.status !== 0) throw new Error("cannot fingerprint source checkout");
	const hash = createHash("sha256");
	for (const relative of listed.stdout.toString().split("\0").filter(Boolean).sort()) {
		const file = path.join(root, relative);
		if (existsSync(file) && statSync(file).isFile()) hash.update(`${relative.replaceAll("\\", "/")}\0`).update(readFileSync(file));
	}
	return { status: status.stdout, files: hash.digest("hex"), bundle: existsSync(path.join(root, "benchmarks", "workflow-evaluation", "v1")) ? treeHash(path.join(root, "benchmarks", "workflow-evaluation", "v1")) : null };
}
...
function appendResultLifecycle(file, details, result) {
	for (const question of result.questions ?? []) {
		appendLifecycle(file, "awaiting_scripted_answer", { ...details, questionId: question.id });
		appendLifecycle(file, "running", details);
	}
	const terminal = result.status === "completed" ? "completed" : result.failure === "timeout" || result.status === "timed_out" ? "timed_out" : "failed";
	appendLifecycle(file, terminal, { ...details, failure: result.failure });
	const verified = details.passed ?? (result.status === "completed" && result.verifier?.passed !== false);
...
function dependencyRoots(descriptor) {
	if (descriptor.dependencyPackages) return descriptor.dependencyPackages.map((root) => path.resolve(root));
	return ["pi-compound-engineering", "pi-subagents", "pi-ask-user"].map((name) => path.join(os.homedir(), ".pi", "agent", "npm", "node_modules", name)).filter(existsSync);
}
...
function provision({ sourceRoot, project, stage, side, pairIndex, runRoot, initialize }) {
	...
	return { workspaceRoot, projectDir, input, promptInput, stageInputPath: path.basename(stageInputPath) };
}
...
function comparableProvenance(result, factor) {
	...
	if (factors.has("workflowRevision")) {
		...
		if (Array.isArray(provenance.resources)) provenance.resources = provenance.resources.map(({ name, source }) => ({ name, source }));
	}
	return provenance;
}
...
function provenanceMismatch(baseline, candidate, factor) {
	...
	return fingerprint(comparableProvenance(baseline, factor)) !== fingerprint(comparableProvenance(candidate, factor));
}
...
function disposition(attempts) {
	...
	if (!candidate?.passed && infrastructureFailure([candidate.result])) return "invalid";
...
function planArtifacts(root) {
	...
	return readdirSync(directory).filter((name) => /\.(?:md|html)$/i.test(name)).map((name) => {
		const file = path.join(directory, name);
		const content = readFileSync(file, "utf8");
		const readiness = content.match(/artifact_readiness:\s*([A-Za-z-]+)/i)?.[1] ?? null;
		return { path: path.relative(root, file).replaceAll("\\", "/"), content, readiness, sha: fileSha(file) };
	});
}
...
function workItemFinalization(root) {
	...
	const executable = items.filter((item) =>
		!["epic", "decision"].includes(String(item.type).toLowerCase()) && item.status !== "closed",
	);
	return { passed: executable.length === 0, openExecutable: executable.map((item) => item.id) };
}
...
async function defaultRunSample(sample, descriptor, sourceRoot) {
	...
	if (settings) {
		...
		writeFileSync(path.join(sample.workspaceRoot, ".pi", "settings.json"), `${JSON.stringify(settings, null, 2)}\n`);
	}
	const commandName = sample.stage === "work" ? "work-resume" : `work-${sample.stage}`;
	const argument = sample.stage === "brainstorm" ? sample.promptInput : sample.stageInputPath;
	...
	if (sample.stage === "work" && !customPrompt) {
		prompts = [`/work-migrate ${sample.stageInputPath}`, "/work-goal Resume the active migrated plan through every executable WorkItem, verification, review, commit, and finalization."];
	...
	const output = (rpc.events ?? []).toReversed().find((event) => event.type === "message_end" && event.message?.role === "assistant" && messageText(event.message))?.message;
	...
	let stageGate;
	if (sample.stage === "brainstorm") stageGate = { passed: artifacts.some((artifact) => artifact.readiness === "requirements-only"), artifacts };
	else if (sample.stage === "plan") stageGate = { passed: artifacts.some((artifact) => artifact.readiness === "implementation-ready"), artifacts };
	else if (path.basename(sample.projectDir) === "csv-expenses") stageGate = verifyCsvProject(sample.workspaceRoot);
	else stageGate = await verifyCalculatorProject(sample.workspaceRoot, descriptor.browserRunner ?? null, { evidenceDirectory: sample.evidenceDirectory });
	if (sample.stage === "work") {
		const workItems = workItemFinalization(sample.workspaceRoot);
		stageGate = { ...stageGate, passed: stageGate.passed && workItems.passed, workItems };
	}
	if (stageGate.browser && descriptor.browser?.name) {
		const browserMatches = stageGate.browser.name === descriptor.browser.name && stageGate.browser.version === descriptor.browser.version;
		if (!browserMatches) stageGate = { ...stageGate, passed: false, reason: "browser-fingerprint-mismatch" };
	}
	...
	const metrics = {
		...
		contextTokens: Number.isFinite(usage.contextUsage?.tokens) && Number.isFinite(rpc.initialUsage?.contextUsage?.tokens) ? Math.max(0, usage.contextUsage.tokens - rpc.initialUsage.contextUsage.tokens) : undefined,
		questions: rpc.questions?.length ?? 0,
	};
	const provenanceComplete = Boolean(rpc.provenance?.revision && rpc.provenance?.resources && rpc.provenance?.model && rpc.provenance?.isolation);
	const verifier = { passed: rpc.status === "completed" && provenanceComplete && Boolean(outputText) && Boolean(telemetry) && gitStatus.length === 0 && stageGate.passed, output: Boolean(outputText), telemetry: telemetryError ?? "complete", provenance: provenanceComplete, gitClean: gitStatus.length === 0, stage: stageGate };
	let handoffArtifact = outputText;
	if (sample.stage !== "work") {
		const readiness = sample.stage === "brainstorm" ? "requirements-only" : "implementation-ready";
		handoffArtifact = artifacts.find((artifact) => artifact.readiness === readiness)?.content ?? outputText;
	}
	const screenshots = (stageGate.gates ?? []).filter((gate) => gate.name === "screenshot" && gate.passed).map((gate) => gate.detail);
	return { ...rpc, wallMs, harnessRpcWallMs: Math.max(0, totalRpcWallMs - wallMs), metrics, telemetry, artifacts, screenshots, output: outputText, handoffArtifact, diff: gitStatus.join("\n"), verifier, prompt, prompts };
}
...
function defaultEvaluatePair(baselineArtifact, candidateArtifact, rubric, descriptor) {
	...
	if (payload.length > 524_288) throw new Error("evaluator payload exceeds 512 KiB");
	const promptRoot = mkdtempSync(path.join(os.tmpdir(), "ce-workflow-evaluator-"));
	...
	try {
		...
		const run = spawnSync(piCommand, piArgs, { encoding: "utf8", timeout: evaluator.timeoutMs ?? 900_000, maxBuffer: 16 * 1024 * 1024 });
		...
		if (evaluator.provider && actualProvider && evaluator.provider !== actualProvider) throw new Error("evaluator provider fingerprint mismatch");
		if (evaluator.model && actualModel && !String(actualModel).endsWith(String(evaluator.model).split("/").at(-1))) throw new Error("evaluator model fingerprint mismatch");
		...
		return { scores, control: blinded.control, evaluator: { provider: actualProvider, model: actualModel, wallMs: Date.now() - started, usage: assistant?.usage ?? null } };
	} finally {
...
function infrastructureFailure(results) {
	...
	return failures.find((failure) => /rate-limit|provider|process-error|browser-unavailable/.test(failure)) ?? null;
}

function compactVerdict(verdict) {
	return { ...verdict, pairs: (verdict.pairs ?? []).map(({ pairIndex, order, selectedAttempt, deltas }) => ({ pairIndex, order, selectedAttempt, deltas })) };
}

function loadCalibration(descriptor, sourceRoot) {
	if (!descriptor.calibrationPath) throw new Error("decision mode requires an approved calibrationPath");
	const calibration = readJson(path.resolve(descriptor.calibrationPath), "calibration evidence");
	if (calibration.project !== descriptor.project || calibration.stage !== descriptor.stage) throw new Error("calibration project/stage mismatch");
	if (calibration.bundleSha !== projectBundleHash(projectRoot(sourceRoot, descriptor.project))) throw new Error("calibration bundle is stale");
	if (calibration.baselineFingerprint !== fingerprint(resolvedPair(descriptor).baseline)) throw new Error("calibration environment is stale");
	if (!(calibration.minimumImprovement >= 0.05) || !(calibration.maximumDimensionRegression >= 0.1)) throw new Error("calibration weakens fixed threshold floors");
	return calibration;
}
...
export async function runDecisionExperiment(descriptor, seams = {}) {
	...
	const calibration = descriptor.calibration || seams.skipCalibration ? null : loadCalibration(descriptor, sourceRoot);
	if (calibration) descriptor = { ...descriptor, budgets: { ...descriptor.budgets, tokenCeiling: calibration.tokenCeiling, wallMsCeiling: calibration.wallMsCeiling } };
	...
	try {
		for (let pairIndex = 0; pairIndex < 3; pairIndex += 1) {
			...
			for (let attemptIndex = 0; attemptIndex < 2; attemptIndex += 1) {
				...
				for (const side of order) {
					...
					sample.evidenceDirectory = path.join(controlRoot, "screenshots", `${pairIndex}-${attemptIndex}-${side}`);
					appendLifecycle(lifecycle, "provisioned", { pairIndex, attemptIndex, side, workspaceRoot: sample.workspaceRoot });
					...
					appendLifecycle(lifecycle, "running", { pairIndex, attemptIndex, side });
					...
					writeFileSync(path.join(controlRoot, `raw-${pairIndex}-${attemptIndex}-${side}.json`), `${JSON.stringify(sanitize(raw[side]), null, 2)}\n`);
					appendResultLifecycle(lifecycle, { pairIndex, attemptIndex, side, passed: passed(raw[side], descriptor.budgets) }, raw[side]);
					cleanupWorkspace(sample.workspaceRoot);
					appendLifecycle(lifecycle, "cleaned", { pairIndex, attemptIndex, side });
				}
				if (provenanceMismatch(raw.baseline, raw.candidate, descriptor.factor)) {
					attempts.push({ comparisonFailure: "provenance-mismatch", baseline: decisionSample(raw.baseline, {}, descriptor.budgets), candidate: decisionSample(raw.candidate, {}, descriptor.budgets), raw: sanitize(raw) });
					break;
				}
				...
				if (!passed(raw.baseline, descriptor.budgets) || !passed(raw.candidate, descriptor.budgets)) {
					attempts.push({ baseline: decisionSample(raw.baseline, {}, descriptor.budgets), candidate: decisionSample(raw.candidate, {}, descriptor.budgets), raw: sanitize(raw) });
					break;
				}
				let evaluation;
				try {
				...
				const evaluatorIdentity = { provider: evaluation.evaluator?.provider, model: evaluation.evaluator?.model };
				if (evaluatorEvidence.some((item) => fingerprint({ provider: item.provider, model: item.model }) !== fingerprint(evaluatorIdentity))) {
					attempts.push({ evaluatorFailure: "changed evaluator fingerprint", baseline: decisionSample(raw.baseline, {}, descriptor.budgets), candidate: decisionSample(raw.candidate, {}, descriptor.budgets), raw: sanitize(raw) });
					break;
				}
		...
		const verdict = evaluateDecision(pairs, rubric, { primaryMetric: descriptor.primaryMetric, minimumImprovement: calibration?.minimumImprovement ?? descriptor.minimumImprovement, maximumDimensionRegression: calibration?.maximumDimensionRegression });
		const workflowWallMs = pairs.flatMap((pair) => pair.attempts).reduce((sum, attempt) => sum + (attempt.raw?.baseline?.wallMs ?? 0) + (attempt.raw?.candidate?.wallMs ?? 0), 0);
		const evaluatorWallMs = evaluatorEvidence.reduce((sum, evaluator) => sum + (evaluator.wallMs ?? 0), 0);
		const costs = { workflow: { wallMs: workflowWallMs }, harness: { wallMs: Math.max(0, Date.now() - harnessStarted - workflowWallMs - evaluatorWallMs) }, evaluator: { wallMs: evaluatorWallMs, samples: evaluatorEvidence } };
		const rawSamples = pairs.flatMap((pair) => pair.attempts.flatMap((attempt) => [attempt.raw?.baseline, attempt.raw?.candidate].filter(Boolean)));
		const evidence = sanitize({ contract: "ce-workflow-evidence/v1", runId, mode: "decision", decisionGrade: true, fingerprints: { descriptor: fingerprint(descriptor), calibration: calibration ? fingerprint(calibration) : null, source: before }, declaredFactor: descriptor.factor, costs, testOutput: rawSamples.map((sample) => sample.verifier ?? null), bugs: rawSamples.filter((sample) => !passed(sample, descriptor.budgets) && !infrastructureFailure([sample])).map((sample) => ({ failure: sample.failure, error: sample.error, verifier: sample.verifier })), pairs, evaluator: evaluatorEvidence, verdict });
		const evidencePath = path.join(controlRoot, "evidence.json");
		writeFileSync(evidencePath, `${JSON.stringify(evidence, null, 2)}\n`);
		writeFileSync(path.join(controlRoot, "evaluator-control.json"), `${JSON.stringify(evaluatorControl, null, 2)}\n`);
		const reportPath = path.join(controlRoot, "report.json");
		const infrastructureFailures = rawSamples.filter((sample) => infrastructureFailure([sample])).map((sample) => ({ failure: sample.failure, error: sample.error }));
		writeFileSync(reportPath, `${JSON.stringify({ runId, mode: "decision", status: verdict.status, evidencePath, deterministicFacts: { hardGateStatus: verdict.status, pairCount: pairs.length }, evaluatorJudgments: verdict.qualitative ?? null, workflowCosts: verdict.summary ?? costs.workflow, harnessCost: costs.harness, evaluatorCost: costs.evaluator, infrastructureFailures, invalid: verdict.status === "invalid", benchmarkContractChanged: false, verdict: compactVerdict(verdict) }, null, 2)}\n`);
...
function projectBundleHash(root) { return hashTree(root, (relative) => relative === "goldens/approval.json"); }
...
export function requiresSentinel(changedPaths, declaredChangeType = "") {
	...
	return changedPaths.some((file) => /^(extensions\/work-models\.js|skills\/ce-(brainstorm|plan|work)\/|prompts\/work-|agents\/workItem-)/.test(file.replaceAll("\\", "/")) || !knownNarrow.test(file.replaceAll("\\", "/")));
}
...
export async function runSentinelExperiment(descriptor, seams = {}) {
	...
	if (!seams.skipApproval) validateDescriptor({ ...descriptor, project: descriptor.projects?.[0] ?? "calculator", stage: "brainstorm" });
	...
	const runStage = seams.runStage ?? (async ({ side, stage, input, workspaceRoot, projectDir }) => {
		...
		const stageDescriptor = { ...descriptor, project, stage, calibrationPath: descriptor.calibrations?.[`${project}:${stage}`] };
		if (!stageDescriptor.calibrationPath) throw new Error(`sentinel requires calibration for ${project}:${stage}`);
		const calibration = loadCalibration(stageDescriptor, sourceRoot);
		stageDescriptor.budgets = { ...descriptor.budgets, tokenCeiling: calibration.tokenCeiling, wallMsCeiling: calibration.wallMsCeiling };
		const answers = readJson(path.join(projectDir, "answers.json"), `${project} answer bank`);
		...
		command(workspaceRoot, "git", ["commit", "--quiet", "-m", `benchmark handoff: ${stage}`]);
		const result = await defaultRunSample({ side, stage, promptInput: input, stageInputPath, workspaceRoot, projectDir, answers, evidenceDirectory: path.join(controlRoot, "screenshots", `${project}-${side}-${stage}`) }, stageDescriptor, sourceRoot);
	...
	try {
		for (const project of descriptor.projects ?? ["calculator", "csv-expenses"]) {
			...
			for (const side of descriptor.sides ?? ["baseline", "candidate"]) {
				...
				appendLifecycle(lifecycle, "provisioned", { project, side, workspaceRoot });
				...
				for (const stage of ["brainstorm", "plan", "work"]) {
					...
					writeFileSync(path.join(controlRoot, `checkpoint-${project}-${side}.json`), `${JSON.stringify(projectResult, null, 2)}\n`);
		...
		const reportPath = path.join(controlRoot, "report.json");
		const projectSummary = results.map((result) => ({ project: result.project, side: result.side, stages: result.stages.map((stage) => ({ stage: stage.stage, status: stage.status, failure: stage.failure })), acceptance: result.acceptance?.passed ?? false }));
		writeFileSync(reportPath, `${JSON.stringify({ runId, mode: "sentinel", status, evidencePath, projects: projectSummary }, null, 2)}\n`);
		...
		return { runId, mode: "sentinel", status, projects: results, evidencePath, reportPath };
	} finally { rmSync(runRoot, { recursive: true, force: true }); }
}
...
export async function runCalibrationExperiment(descriptor, seams = {}) {
	...
	const stable = ["candidate-accepted", "quality-pass-no-cost-win"].includes(decision.status);
	if (!stable || !decision.verdict?.pairs || decision.verdict.pairs.length !== 3) return { ...decision, mode: "calibration", status: "invalid-calibration" };
	...
	const sourceRoot = path.resolve(seams.sourceRoot ?? defaultSourceRoot);
	const calibrationRecord = { project: descriptor.project, stage: descriptor.stage, bundleSha: projectBundleHash(projectRoot(sourceRoot, descriptor.project)), baselineFingerprint: fingerprint(resolvedPair(descriptor).baseline), generatedAt: new Date().toISOString(), ...calibration };
	const calibrationPath = path.join(path.dirname(decision.evidencePath), "calibration.json");
	writeFileSync(calibrationPath, `${JSON.stringify(calibrationRecord, null, 2)}\n`);
...
export function runGoldenUpdate(descriptor, seams = {}) {
	...
	if (descriptor.humanApproved && (!descriptor.approvedBy || !descriptor.acceptancePassed || !descriptor.acceptanceEvidence)) throw new Error("approved golden updates require approver and retained acceptance evidence");
	...
	const manifestFile = path.join(sourceRoot, "benchmarks", "workflow-evaluation", "v1", "manifest.json");
	...
	return { mode: "golden-update", status: "approved", approvalPath: approvalFile, manifestPath: manifestFile, evidencePath };
}
...
export async function runSmokeExperiment(descriptor, seams = {}) {
	...
	const sourceRoot = path.resolve(seams.sourceRoot ?? defaultSourceRoot);
	const calibration = descriptor.calibrationPath ? loadCalibration(descriptor, sourceRoot) : null;
	if (calibration) descriptor = { ...descriptor, budgets: { ...descriptor.budgets, tokenCeiling: calibration.tokenCeiling, wallMsCeiling: calibration.wallMsCeiling } };
	...
	try {
		for (const side of ["baseline", "candidate"]) {
			...
			const attempt = sanitize({ side, pairIndex: 0, passed: ok, failure, result });
			attempts.push(attempt);
			writeFileSync(path.join(controlRoot, `attempt-${side}.json`), `${JSON.stringify(attempt, null, 2)}\n`);
		...
		if (provenanceMismatch(attempts[0]?.result, attempts[1]?.result, descriptor.factor)) status = "invalid";
		let evaluation = null;
		let evaluatorFailure = null;
		if (status === "diagnostic-pass") {
			try {
				evaluation = await (seams.evaluatePair ? seams.evaluatePair(attempts[0].result, attempts[1].result, rubric, { pairIndex: 0, attemptIndex: 0 }) : defaultEvaluatePair(attempts[0].result.artifacts ?? attempts[0].result.output ?? "", attempts[1].result.artifacts ?? attempts[1].result.output ?? "", rubric, { ...descriptor, sourceRoot }));
				if (evaluation.control) writeFileSync(path.join(controlRoot, "evaluator-control.json"), `${JSON.stringify(evaluation.control, null, 2)}\n`);
				const baselineScores = evaluation.scores.baseline;
				const candidateScores = evaluation.scores.candidate;
				const qualityRegressed = dimensions.reduce((sum, dimension) => sum + candidateScores[dimension] - baselineScores[dimension], 0) < 0 || rubric.criticalDimensions.some((dimension) => candidateScores[dimension] < baselineScores[dimension]);
				const baselineQuestions = (attempts[0].result.questions ?? []).filter((question) => question.expected === false).length;
				const candidateQuestions = (attempts[1].result.questions ?? []).filter((question) => question.expected === false).length;
				if (qualityRegressed || candidateQuestions > baselineQuestions) status = "diagnostic-candidate-rejected";
			} catch (error) {
				evaluatorFailure = error instanceof Error ? error.message : String(error);
				status = "invalid";
			}
		}
		const workflowWallMs = attempts.reduce((sum, attempt) => sum + (attempt.result?.wallMs ?? 0), 0);
		...
		const evidence = sanitize({
			...
			mode: "smoke",
			decisionGrade: false,
			fingerprints: { descriptor: fingerprint(descriptor), calibration: calibration ? fingerprint(calibration) : null, source: before, sides: Object.fromEntries(attempts.map((item) => [item.side, fingerprint(descriptor[item.side])])) },
			declaredFactor: descriptor.factor,
			costs: { workflow: attempts.map((attempt) => ({ side: attempt.side, metrics: attempt.result?.metrics ?? attempt.result?.usage ?? null })), harness: { wallMs: Math.max(0, Date.now() - harnessStarted - workflowWallMs - evaluatorWallMs) }, evaluator: evaluation?.evaluator ?? null },
			evaluator: evaluation ? { scores: evaluation.scores, evaluator: evaluation.evaluator } : null,
			evaluatorFailure,
			...
			testOutput: attempts.map((item) => ({ side: item.side, verifier: item.result?.verifier ?? null })),
			bugs: attempts.filter((item) => item.failure && !infrastructureFailure([item.result])).map((item) => ({ side: item.side, failure: item.failure, error: item.result?.error })),
		...
		const evidencePath = path.join(controlRoot, "evidence.json");
		writeFileSync(evidencePath, `${JSON.stringify(evidence, null, 2)}\n`);
		const infrastructureFailures = attempts.filter((item) => infrastructureFailure([item.result])).map((item) => ({ side: item.side, failure: item.failure, error: item.result?.error }));
		writeFileSync(path.join(controlRoot, "report.json"), `${JSON.stringify({ runId, mode: "smoke", decisionGrade: false, status, evidencePath, deterministicFacts: attempts.map(({ side, passed, failure }) => ({ side, passed, failure })), evaluatorJudgments: evaluation?.scores ?? null, workflowCosts: evidence.costs.workflow, harnessCost: evidence.costs.harness, evaluatorCost: evidence.costs.evaluator, infrastructureFailures, invalid: status === "invalid", benchmarkContractChanged: false }, null, 2)}\n`);
...
async function main() {
	if (process.argv.includes("--help") || !process.argv[2]) { process.stdout.write(`${usage()}\n`); return; }
	let descriptor;
	...
	else if (descriptor.mode === "decision") result = await runDecisionExperiment(descriptor);
...
if (process.argv[1] === fileURLToPath(import.meta.url)) main().catch((error) => { process.stderr.write(`${error instanceof Error ? error.message : String(error)}\n`); process.exitCode = 1; });
