...
try {
	...
	assert.equal(
		...
		undefined,
		"the persisted baseline excludes its verifier batch",
	);
	assert.equal(
		...
		"background verification is still queued or running",
		"a different baseline keeps the queued verifier visible",
	);
} finally {
...
assert.equal(
	mod.workGoalBaselineHead({ startedAt: Date.now() }),
	undefined,
