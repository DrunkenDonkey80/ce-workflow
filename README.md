...
| `/work-stop [reason]` | Stop autonomous work cleanly | Marks any active work (resume loop, project goal, or inline slice) as stopping, asks the agent to checkpoint and stop at the next safe phase boundary, then leaves it resumable with `/work-resume`. (`/work-resume-stop` is a kept alias) |
...
| `/work-goal [--tokens 100k] <objective>` | Run a raw autonomous goal | Extension command: appends goal-management rules, microcompacts before continuations, retries transient provider/context errors, caps optional token budgets, rejects contradictory completion summaries, uses `ask_user` for interactive decisions, and pauses only when interactive prompting is unavailable; only `/work-goal resume [answer]` resumes a paused goal |
...
17. `/work-goal` runs a session-scoped autonomous loop with `ask_user` decision gates, microcompaction, retryable provider/context-error recovery, optional budgets, and an inline-first/no-agent-list system policy. Ordinary chat never starts or resumes a persisted goal; use `/work-goal resume [answer]` explicitly.
...
`/work-context` is separate from model choice and uses no model call for its compact summary. `/work-resume` is an autonomous slice loop: each slice gets a fresh-context boundary, and the loop continues to the next ready Bead until a decision, a blocker, epic completion, or `/work-stop`. Use `/work-goal` explicitly for cross-session autonomous multi-step loops; those loops microcompact before continuations and can be stopped with `/work-stop`. For normal inspection, use `/work-report` and `/work-resume`; avoid raw epic JSON.
