...
- when the handoff names an initiative, read `work-helper.mjs initiative-summary <initiative-id>`, select exactly one needs-plan child epic, and scope all planning mutations to that child; an initiative is aggregate state, never an executable target, and sibling epics must not be planned or started;
