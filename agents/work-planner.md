...
- compare the master plan against existing open, in-progress, and closed children once, then freeze one finite backlog; execution must never trigger another broad gap-finding audit;
...
- create every currently known executable item needed for that finite backlog, always with `--parent <roadmap-id>`, and create durable blockers for external requirements; do not defer known work to a later planner rediscovery pass;
- batch defects that share an owning production surface and verification command instead of creating one micro-slice per defect; every executable item must name its estimated implementation paths in one `Files changed:` note and contain at most 8 such files, matching finish-task's coded default cap; inspect and re-cut before creation when the estimate is larger; if the planning work item says `big slice` or `wo:execution-agent`, include `wo:execution-agent` in every executable child's notes so resume keeps the high-risk writer boundary;
- every executable item must cite an original requirement or acceptance example and concrete failing evidence; after the finite backlog is exhausted, later work requires an explicit failing bug or user-added scope, never another generic audit;
- append `wo:finite-backlog` to the planning item before closing it so the coded resume gate stops tail-chasing;
...
- require browser, device, or provider-specific evidence only when the project contract requires it or the configured provider's documented/runtime capability was verified during planning. Never require a trace archive, video, or provider-specific artifact speculatively; use supported screenshots, DOM assertions, logs, and state checks instead, or create a blocker when no adequate supported proof exists;
...
- before accepting the handoff, run `git status --porcelain=v1 --untracked-files=all` and fail `BLOCKED` if any path outside `.ce-workflow/work-items.json`, workflow runtime artifacts, and the explicitly requested dated plan artifact differs from the launch baseline named in the handoff. A new unstaged tracked `AGENTS.md` modification may be observed without stopping only when the parent baseline explicitly marks the managed overlay eligible; do not modify, stage, or revert it, and require its final worktree hash to equal both parent-supplied AGENTS hashes. Preexisting, staged, untracked, still-modified, or hash-mismatched AGENTS dirt and every unrelated new path remain blocking. Never attest no source changes without these checks;
...
- blockers, if any;
- final line: for initiative broad-plan preparation, name the coded suggested next planning action and do not name `/wo → Resume work`; otherwise use `Next: /wo → Resume work <roadmap-id>` when executable work exists, or `Next: roadmap <roadmap-id> "<title>" is complete; close it explicitly with /wo → Roadmaps close <roadmap-id>.` when no work remains.
