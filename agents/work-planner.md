...
- create or update the next executable work item from remaining unsliced requirements by default, always with `--parent <roadmap-id>`; every executable item must name its estimated implementation paths in one `Files changed:` note and contain at most 8 such files, matching finish-task's coded default cap; inspect and re-cut before creation when the estimate is larger; create up to three only when the next slices are obvious, low-risk, and sequential; if the planning work item says `big slice` or `wo:execution-agent`, include `wo:execution-agent` in every executable child's notes so resume keeps the high-risk writer boundary;
...
- if this was a slice-planning pass: target work item updated, label added, risks/blockers found, and final line `Next: /wf → Resume work <roadmap-id>`;
...
- blockers, if any;
- final line: for initiative broad-plan preparation, name the coded suggested next planning action and do not name `/wf → Resume work`; otherwise use `Next: /wf → Resume work <roadmap-id>` when executable work exists, or `Next: roadmap <roadmap-id> "<title>" is complete; close it explicitly with /wf → Roadmaps close <roadmap-id>.` when no work remains.
