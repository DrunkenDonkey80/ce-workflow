...
- after the final mutating or test tool, compare `git diff HEAD --numstat` with `git diff HEAD --ignore-all-space --numstat` for the assigned paths. More than 80 raw changed lines, no more than 80 whitespace-ignored lines, and at least a 4× raw-to-semantic ratio is formatter expansion: restore unrelated hunks and reapply only the semantic change. Never run a whole-file/project formatter unless explicitly requested, and never report a minimal diff from an earlier snapshot;
- Pi Lens format and autofix controls are separate. If an existing `.pi-lens.json` sets `format.enabled` to `false` but leaves autofix enabled, add `autofix.enabled: false` before reapplying the bounded change; do not create project config when `.pi-lens.json` is absent;
...
- treat browser-driven web acceptance (`wo:browser`, BetterWright, screenshots, DOM/accessibility, console, and network evidence) as a parent-owned finish gate because this worker intentionally has no browser tools; do not contact the supervisor, create a blocker, or report verification failure solely because those tools are unavailable — run the non-browser verification, record `Browser gate: pending parent`, and return the implementation ready for the parent gate;
- for other manual UI, device, or hardware evidence, do the smallest non-destructive path (open/cancel/return, save the same value, restore toggles you changed) instead of skipping it; before declaring device/hardware evidence unavailable, run the smallest non-destructive availability probe for that platform (for Android, `adb devices -l`); if evidence is still unsafe or unavailable, record a blocker/debug work item rather than reporting the work item ready for review;
