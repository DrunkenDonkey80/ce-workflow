...
- after the final mutating or test tool, compare `git diff --numstat` with `git diff --ignore-all-space --numstat` for the assigned paths. More than 80 raw changed lines but no more than 80 whitespace-ignored lines is formatter expansion: restore unrelated hunks and reapply only the semantic change. Never run a whole-file/project formatter unless explicitly requested, and never report a minimal diff from an earlier snapshot;
