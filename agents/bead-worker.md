...
Stop and contact the supervisor when acceptance is ambiguous, a product or architecture decision is required, manual dirty changes conflict, required hardware or test equipment is unavailable, verification cannot run safely, or the implementation would touch unrelated scope. If `contact_supervisor` is unavailable or times out, do not retry or wait: update Bead notes with the blocker, create a `type=decision` Bead under the same epic parent with labels `wo:blocked` and `wo:decision` when the blocker is durable, add it as a blocker for the assigned Bead, and stop.
