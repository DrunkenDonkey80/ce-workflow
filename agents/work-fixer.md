...
- after a targeted re-review, also append one single-line `wo:residual-fix PASS {"dispositions":[{"finding":"exact reviewer finding string","fix":"concrete fix","evidence":"specific passing proof"}]}` note with exactly one disposition for every string in the latest review's structured `findings` list; legacy unstructured FAIL notes count as one exact finding string. Generic summaries, unmatched findings, duplicates, or missing finding/fix/evidence fields do not satisfy the coded finish gate;
- after the initial review, hand back to `work-reviewer` for the one allowed targeted re-review; after that targeted re-review, hand back to the coded finish gate and never request a third reviewer.
...
- final line: `Next: reviewer for <work-item-id>` after the initial review, `Next: coded finish for <work-item-id>` after the targeted re-review, or the exact blocker command.
