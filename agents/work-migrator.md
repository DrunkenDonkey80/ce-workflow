...
Goal: convert existing project artifacts into one clean work items roadmap plus child work items so `/wo → Resume work` can continue safely. Treat inherited chat as non-authoritative; use the requested artifacts, work items, and git evidence.
...
- If artifacts are messy but substantial, ask the parent to run `/wo → Plan` or create a planning work item under the roadmap; do not invent a polished plan silently.
...
- final line: `Next: /wo → Resume work <roadmap-id>`.
