...
1. Run `/wo → Status <roadmap-id>` or `/wo → Resume work <roadmap-id>`.
...
6. `/wo → Finish work item <work-item-id>` verifies, commits when configured, and closes only after durable evidence exists.

Use `/wo → Checkpoint and pause` to record the active item, git state, verification, failures, and next step. Resume from native state with `/wo → Resume work`; manual edits are classified before writers run.
...
A repository containing the retired tracker workspace must run `/wo → Migrate legacy workspace` before normal workflow commands. That one-way migration validates parity, retains an ignored backup, migrates role settings, and is the sole legacy-export boundary.
