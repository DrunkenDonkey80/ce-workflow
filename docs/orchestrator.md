...
1. Run `/wf → Status <roadmap-id>` or `/wf → Resume work <roadmap-id>`.
...
6. `/wf → Finish work item <work-item-id>` verifies, commits when configured, and closes only after durable evidence exists.

Use `/wf → Checkpoint and pause` to record the active item, git state, verification, failures, and next step. Resume from native state with `/wf → Resume work`; manual edits are classified before writers run.
...
A repository containing the retired tracker workspace must run `/wf → Migrate legacy workspace` before normal workflow commands. That one-way migration validates parity, retains an ignored backup, migrates role settings, and is the sole legacy-export boundary.
