...
`/wo → Small task`, `/wo → Medium task`, `/wo → Large task`, and `/wo → Auto-route task` create correctly scoped work. `/wo → Resume work` starts the extension-owned autonomous loop and advances one ready item at a time until the selected scope completes or needs a real decision. `/wo → Add work`, `/wo → Debug`, and `/wo → Checkpoint and pause` preserve the same graph through interruptions. `/wo → Status` and `/wo → Blocker report` are cheap deterministic projections; `F9 → Fleet` shows the main-chat orchestrator as the root above its direct, prefetch, and verifier agents.
...
New repositories need no tracker installation. Existing legacy workspaces are explicitly migrated with `/wo → Migrate legacy workspace`; it is the only legacy boundary and normal native commands do not fall back to it.
