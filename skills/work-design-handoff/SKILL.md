...
- Preserve its target, variant, screen, state, and viewport scope. For v2 handoffs, map every required target to its named variant and preview route/artifact; do not collapse desktop and mobile into one responsive claim.
...
Capture every required fidelity cell at the approved handoff hash and current target revision. Verify every v2 target/variant at its declared viewport and input modes, including the declared preview route/artifact. Inspect hierarchy/signature, tokens, required regions and content, responsive reflow, visible states, clipping/overlap, focus, and reduced motion. Do not require raw pixel equality.
