...
When `ce-debug` cannot safely fix or verify, record the debugger's failure artifact in the bug work item, create a `type=decision` work item for the human question when needed, label the bug `wo:blocked`, and add a dependency from the bug to that decision. `F7 → Resume work` should then continue with the next unrelated ready work item from `node scripts/work-helper.mjs work-ready-summary`; it must not spin on blocked work. `F7 → Blocker report` is the human handoff surface for blocked/debug-needed work items.
