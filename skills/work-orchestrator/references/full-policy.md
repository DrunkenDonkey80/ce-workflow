...
3. Claim the Bead during coded intake, then implement directly in the current session with targeted reads.
...
5. Do not list or launch agents for routine `/work-small`. If the coded finalizer detects sensitive paths, launch exactly one `bead-reviewer`, persist PASS evidence, then rerun with `--reviewed`.
6. If scope exceeds two implementation files, verification fails, acceptance is unclear, or dirty files conflict, leave the Bead open and route the next invocation through med/debug rather than silently cutting scope.
...
1. Resolve the active epic first; if ambiguous, ask.
2. Create and claim one executable Bead directly; do not create a planning Bead or planner agent for an already explicit bounded request.
3. Implement directly in the current session with a limit of eight implementation files.
4. Use `finish-task --max-files 8` for verification and coded commit/close. Launch exactly one `bead-reviewer` only when the coded risk gate, ambiguous acceptance, or failed evidence requires independent review.
5. If semantic slicing or architecture decisions are genuinely required, stop the inline path and route to big/planner rather than inventing child Beads.

Durable scope still goes into Beads; a short in-session checklist is enough for routine medium work.
...
3. Launch `bead-planner` to split that slice into the next executable Bead by default, plus any decision Beads; create up to three executable Beads only for obvious low-risk sequential work. Mark executable descendants with `wo:execution-agent` in notes so resume preserves the big/risky writer boundary.
...
7. Commit and close through the coded `finish-task` gate after verification and review pass. Use `bead-committer` only when unusual repository policy cannot be represented by that gate. Remove or satisfy blocking dependencies so `/work-resume` can continue downstream work.
...
8. If it is an implementation Bead, apply the coded execution policy. Clear bounded work runs directly in the current session and finishes with `finish-task`; high-risk markers (security/auth, persistence/migration, payments, concurrency, breaking API/ABI, destructive production work, or explicit big-mode provenance) launch exactly `bead-worker` with `context:fresh`.
9. Skip an independent reviewer when acceptance is explicit, deterministic verification passed, and the bounded diff is non-sensitive. Launch exactly one `bead-reviewer` for high-risk, large, UI-acceptance, hardware/live-evidence, ambiguous, or failed/missing-verification changes. Never call `subagent list`; the extension resolves the exact role in code.
10. If review returns `FAIL` for fixable code, launch `bead-fixer` once with exact findings, then review again. If root-cause debugging is needed, create/reuse a `wo:debug` bug Bead and stop so the next `/work-resume` selects `bead-debugger`.
11. Commit, close, amend Beads state, and push through the coded finish helper. Do not launch `bead-committer` for routine work; reserve it only for unusual repository commit policy that cannot be represented by the coded gate.
...
Work inline in the current session unless the coded policy requires specialization. Do not call `subagent list` during a workflow. When needed, launch the exact package role (`bead-planner`, `bead-worker`, `bead-reviewer`, `bead-fixer`, `bead-debugger`, `bead-migrator`, or `bead-advisor`) directly; do not substitute builtin roles. Children get concrete Bead IDs and must not launch their own subagent workflows. Always use fresh context unless the user explicitly requests inherited history. Use `outputMode: "file-only"` with a short relative output filename for review/research/work outputs unless the complete result is under about 20 lines; do not pass `.pi-subagents/` paths because the subagent tool owns the artifact directory. Keep only a short structured summary in the parent; do not paste long tool logs, full `bd show` epic JSON, raw `bd ready --json`, raw `bd children --json`, or whole master plans back into the control session. Pipe Beads JSON through python/node projections when only IDs/status/titles are needed.
...
- create the next executable Bead under the epic (`--parent <epic-id>`) by default, only when no existing open/in-progress/closed child already covers that implementation unit; create up to three executable Beads only when the next units are obvious, low-risk, and sequential; propagate `wo:execution-agent` from a big/risky planning Bead into each executable child's notes;
...
- report `PASS` or `FAIL` with evidence and append the same compact `wo:review PASS|FAIL` verdict to the assigned Bead so coded resume/finish can advance;
