...
- **resume** — autonomous slice loop over the finite backlog created at the first roadmap planning boundary. Run each ready work item inline by default (set `sliceExecutionMode=agent` in `/wo → Settings` to route each slice to an isolated `work-worker`); exact debugger/reviewer only when policy requires it. After a slice closes, continue to the next ready item. When the backlog is exhausted, stop as a done candidate instead of launching another gap-finding planner; later work requires an explicit failing bug or user-added scope.
- **goal** — autonomous current-session loop with on-demand microcompaction. Work inline; exact specialists only for the cases above. Completion requires verified evidence. Stop with `/wo → Stop safely`.
- **improve** — read-only necessity/safety pass first. Coded guards block source mutation until every snapshot item is marked `wo:improvement-safety SAFE`, or `APPROVED` after a recorded `ask_user` approval. Potential data, security, API/schema, history/release, external-side-effect, protection-removal, and broad-default changes require approval; two unchanged continuations pause instead of looping.
